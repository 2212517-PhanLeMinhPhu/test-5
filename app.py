...
def get_vpd_chart(df, v_min, v_max):
    ...
    points = base.mark_circle(size=60, color='#2E7D32').encode(
        y=alt.Y('VPD (kPa):Q'),
        tooltip=[alt.Tooltip('Hiển thị Giờ:N', title='Giờ'), alt.Tooltip('VPD (kPa):Q', title='Mức VPD')]
    )
    ...
    def live_monitor():
        ...
        with st.container(border=True):
            ...
            if st.session_state.stt_counter == 0:
                ...
                if v_res < v_min:
                    lbl, color = "🟦 QUÁ ẨM", "#0068C9"
                elif v_res <= v_max:
                    lbl, color = "🟩 LÝ TƯỞNG", "#2E7D32"
                else:
                    lbl, color = "🟥 QUÁ KHÔ", "#FF4B4B"
