...
def process_data_columns(df_raw, c_time, c_temp, c_humi):
    df = pd.DataFrame()
    df["datetime_internal"] = pd.to_datetime(df_raw[c_time].astype(str).str.strip(), errors='coerce', utc=False)
...
def render_realtime_analytics_panel():
    ...
    t1, t2, t3 = st.tabs(["Grid Biểu đồ", "Phân tích buổi", "Bảng Nhật ký"])
    with t1:
