    ...
    with tr:
        with st.container(border=True):
            ...
            t_filter = st.selectbox("📆 Chế độ lọc và gộp:", [
                "📊 Xem toàn bộ dữ liệu gốc", "📆 Tự chọn ngày cụ thể", 
                "🗓️ Chọn 1 tháng (29 ngày)", "📅 Chọn 1 tuần (6 ngày)", 
                "⏱️ 1 Ngày gần nhất (Gom 10p)", "📅 1 Tuần gần nhất (Gom ngày)", 
                "🗓️ 1 Tháng gần nhất (Gom ngày)"
            ])
    ...
    if u_file:
        try:
            ...
            with st.expander("🔍 BẤM VÀO ĐÂY ĐỂ XEM THÔNG SỐ VÀ DỮ LIỆU THÔ", expanded=False):
                ...
                st.markdown("**3. Thống kê thông số cơ bản (Chỉ tính các cột số):**")
                try:
                    st.dataframe(df_up.describe(), use_container_width=True)
                except Exception:
                    st.caption("Không có cột số liệu chuẩn để thống kê.")
            ...
            valid_cols = [c for c in cols if df_up[c].notna().any()]
            if not valid_cols:
                valid_cols = cols
            ...
            for c in valid_cols:
                ...
                if any(k in cl for k in ['time', 'thời gian', 'giờ', 'gio', 'date', 'timestamp', 'created_at']): 
                    detected_time = c
                elif any(k in cl for k in ['tempkk', 'temp', 'temperature', 'nhiệt độ', 'nhiet do', 't°', 't1']): 
                    detected_temp = c
                elif any(k in cl for k in ['humikk', 'hum', 'humidity', 'độ ẩm', 'do am', 'rh', 'h1']): 
                    detected_humi = c
            ...
            with cc2: 
                c_temp = st.selectbox("Cột chứa Nhiệt độ:", cols, index=cols.index(detected_temp) if detected_temp in cols else 0)
            with cc3: 
                c_humi = st.selectbox("Cột chứa Độ ẩm:", cols, index=cols.index(detected_humi) if detected_humi in cols else 0)
            ...
            if df_rc["Nhiệt độ (°C)"].isna().all() or df_rc["Độ ẩm (%)"].isna().all():
                st.error("⚠️ Lỗi khớp kiểu số dữ liệu! Bạn hãy kiểm tra lại xem đã chọn đúng cột chứa số liệu (ví dụ: tempKK, humiKK) chưa nhé.")
                st.stop()
