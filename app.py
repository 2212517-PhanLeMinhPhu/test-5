...
# --- CẤU HÌNH THÔNG TIN KẾT NỐI (DISCORD WEBHOOK) ---
MQTT_BROKER = "broker.hivemq.com"
MQTT_PORT = 1883
MQTT_TOPIC = "vuon_thong_minh/duy_tran/sensors"

# Thay thế link Webhook của kênh Discord của bạn vào đây
DISCORD_WEBHOOK_URL = "https://discord.com/api/webhooks/1234567890/ABCabcdefg_YOUR_WEBHOOK_KEY"
...
def send_discord_auto(message):
    if not DISCORD_WEBHOOK_URL or "YOUR_WEBHOOK_KEY" in DISCORD_WEBHOOK_URL:
        return
    try: 
        # Discord Webhook nhận payload có key cấu trúc là "content"
        requests.post(DISCORD_WEBHOOK_URL, json={"content": message}, timeout=2)
    except: 
        pass
...
def process_incoming_data(df_new):
    ...
    for _, row in df_new.iterrows():
        ...
        if t_col in row and h_col in row:
            ...
            # Cấu trúc chữ đậm của Discord sử dụng cặp dấu song tinh (**) thay vì đơn tinh (*)
            msg = (
