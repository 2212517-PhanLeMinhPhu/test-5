...
def setup_next_day():
    try:
        ...
        if current_dt.hour == 0 and current_dt.minute == 0: next_dt = current_dt + timedelta(hours=7)
        else:
...
def trigger_new_data(v_min, v_max):
    try:
        ...
        # GIẢI QUYẾT MEMORY LEAK: Cắt tỉa lịch sử tối đa 1000 records
        MAX_HISTORY = 1000
...
# --- CACHE DATA ĐỂ GIẢI QUYẾT XỬ LÝ DƯ THỪA (REDUNDANT PROCESSING) ---
    ...
    def live_monitor():
        ...
        with st.container(border=True):
            ...
            if st.session_state.stt_counter == 0:
                ...
                # Lấy trực tiếp từ lịch sử đã sắp xếp
                ...
                if t_tp == "danger_red": st.markdown(f"<div class='danger-
